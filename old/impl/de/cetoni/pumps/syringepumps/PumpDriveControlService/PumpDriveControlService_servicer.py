...
:date: (last modification) 2021-10-01T05:25:57.829535
...
class PumpDriveControlService(PumpDriveControlService_pb2_grpc.PumpDriveControlServiceServicer):
    ...
    def InitializePumpDrive_Result(self, request, context: grpc.ServicerContext) \
        ...
        logging.debug(
            "InitializePumpDrive_Result called in {current_mode} mode".format(
                current_mode=('simulation' if self.simulation_mode else 'real')
            )
        )
        try:
            return self.implementation.InitializePumpDrive_Result(request, context)
        except (DeviceError, SiLAError) as err:
            if isinstance(err, DeviceError):
                err = QmixSDKSiLAError(err)
            err.raise_rpc_error(context=context)
